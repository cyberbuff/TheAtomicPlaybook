...
class JupyterCells:
    ...
    def quick_initialize_markdown(value: [str]):
        return JupyterCells(type=JupyterCells.Type.MARKDOWN,
                            value=["\n".join(value)])

    @staticmethod
    def quick_initialize_code(value: [str]):
        return JupyterCells(type=JupyterCells.Type.CODE,
                            value=["\n".join(value)])
...
class AttackTechnique:
    def __init__(self, stix_object, atomic_object=None):
        ...
        # Note: Shield is not mapped to sub techniques yet. There might be
        # issues because of that. Working on that.
    ...
    def __repr__(self):
        ...
        if self.shield_object:
            cells.append(JupyterCells.quick_initialize_markdown(
                [self.shield_object.to_markdown()]))
...
class AtomicTest:
    class AtomicDependency:
        def __init__(self, dependency_dict, executor):
            ...
            self.check_pre_req_command = get_value_or_404(
                dependency_dict, "prereq_command")
            self.get_pre_req_command = get_value_or_404(
                dependency_dict, "get_prereq_command")
        ...
        def __repr__(self):
            ...
            markdown.append(
                "```{0}\n{1}\n```".format(
                    ...
                    self.check_pre_req_command))
            markdown.append("##### Get Prereq Commands:")
            markdown.append(
                "```{0}\n{1}\n```".format(
                    self.dependency_executor_name,
                    self.get_pre_req_command))
            return "\n".join(markdown)
    ...
    def __init__(self, atomic_test: dict):
        ...
        self.elevation_required = get_value_or_404(
            atomic_test["executor"], "elevation_required")
        self.dependency_executor_name = get_value_or_404(
            atomic_test, "dependency_executor_name")
        if get_value_or_404(atomic_test, "dependencies"):
            self.dependencies = [
                AtomicTest.AtomicDependency(
                    i, self.dependency_executor_name) for i in atomic_test["dependencies"]]
        else:
            self.dependencies = None

        if get_value_or_404(atomic_test, "input_arguments"):
            input_args = atomic_test["input_arguments"]
            self.command = self.replace_command_with_input_args(
            ...
            if(get_value_or_404(atomic_test, "dependencies")):
                for i in self.dependencies:
                    ...
                    i.check_pre_req_command = self.replace_command_with_input_args(
                        i.check_pre_req_command, input_args)

    def __repr__(self):
        """ Converts AtomicTest to [JupyterCell] object"""
        cells = []
        markdown = []
        markdown += ["### Atomic Test #{0} - {1}".format(
            self.test_number, self.name), self.description]
        if self.platforms:
            markdown.append(
        ...
        if self.executor == "manual":
            ...
            if self.dependencies:
                markdown.append(
                    "#### Dependencies:  Run with `{0}`!".format(
                        self.dependency_executor_name))
                markdown.append("\n".join(i.__repr__()
                                          for i in self.dependencies))
                cells.append(
            ...
            cells.append(
                JupyterCells.quick_initialize_code(
                    value=[f'Invoke-AtomicTest {self.technique_id} -TestNumbers {self.test_number}']))
    ...
    def replace_command_with_input_args(
            self, command: str, input_arguments: dict):
